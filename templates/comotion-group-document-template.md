...
Set `theme:` in your frontmatter to one of: `comotion`, `comotion-ai`, or `seedanalytics`. All three themes support **document** and **presentation** modes and sensitivity levels (public, internal, confidential, secret). Generate PDF or Word output using `scripts/md_to_pdf.py` with `--themes-dir` pointing at `claude_skills/md-document/themes`.
